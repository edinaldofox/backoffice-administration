...
exports.read = function(req, res) {
    if(req.assetsCategory.company_id === req.token.company_id)
        res.json(req.assetsCategory);
    else
...
exports.update = function(req, res) {
    ...
    var updateData = req.assetsCategory;
    ...
    if(req.body.company_id === req.token.company_id){
        updateData.updateAttributes(req.body).then(function(result){
            ...
            if(err.name === "SequelizeUniqueConstraintError"){
                ...
                winston.error("Updating assets category failed with error: ", err);
                return res.status(400).send({message: 'An error occurred while editing assets category item. '+err.errors[0].message}); //another error occurred. return sequelize error message
            }
        });
    }
    else{
...
exports.delete = function(req, res) {
    var deleteData = req.assetsCategory;
        ...
        if (result) {
            if (result && (result.company_id === req.token.company_id)) {
                result.destroy().then(function() {
                    ...
                    winston.error("Deleting asset category failed with error: ", err);
...
exports.list = function(req, res) {
    ...
    DBModel.findAndCountAll(
        ...
        winston.error("Getting assets category list failed with error: ", err);
        res.jsonp(err);
    });
};
...
exports.dataByID = function(req, res, next, id) {
    ...
    DBModel.find({
        ...
        if (!result) {
            ...
            req.assetsCategory = result;
            next();
            return null;
        }
    }).catch(function(err) {
        winston.error("Getting assets category data failed with error: ", err);
        return next(err);
    });
};
