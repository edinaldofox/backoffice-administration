        ...
        if (device) {
            if (req.auth_obj.boxid == device.device_id) {
                ...
                device.updateAttributes({
                    ...
                    responses.sendv3(req, res, response_data, 200, 1, 'OK_DESCRIPTION', 'OK_DATA', 'no-store');
                    return null;
                }).catch(function (error) {
                    winston.error("Updating a device's record failed with error: ", error);
                    responses.sendv3(req, res, [], 706, -1, 'DATABASE_ERROR_DESCRIPTION_2', 'DATABASE_ERROR_DATA', 'no-store');
                });
            }
            else {
                responses.sendv3(req, res, [], 705, -1, 'DUAL_LOGIN_ATTEMPT_DESCRIPTION', 'DUAL_LOGIN_ATTEMPT_DATA', 'no-store'); //same user try to login on another device
                return null;
            }
        }
        else {
            ...
            models.devices.upsert({
                ...
                responses.sendv3(req, res, [], 706, -1, 'DATABASE_ERROR_DESCRIPTION_3', 'DATABASE_ERROR_DATA', 'no-store');
            });
        }
        return null;
    });
