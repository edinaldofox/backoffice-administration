...
exports.read = function(req, res) {
    if(req.assetsDetails.company_id === req.token.company_id)
        res.json(req.assetsDetails);
    else
...
exports.update = function(req, res) {
    ...
    var updateData = req.assetsDetails;
        ...
        updateData.updateAttributes(req.body).then(function(result){
            if(deletefile) {
                fs.unlink(deletefile, function (err) {
                    winston.error("Error deleting file on assets details controller: ", err);
                    res.status(400).send({message: 'An error occurred while editing asstes details. '+err.errors[0].message});
                });
            }
            res.json(result);
        }).catch(function(err) {
            if(err.name === "SequelizeUniqueConstraintError"){
                ...
                return res.status(400).send({message: 'An error occurred while editing assets details. '+err.errors[0].message}); //another error occurred. return sequelize error message
            }
        });
    }
    else{
...
exports.delete = function(req, res) {
    var deleteData = req.assetsDetails;
        ...
        if (result) {
            if (result && (result.company_id === req.token.company_id)) {
                result.destroy().then(function() {
                    ...
                    winston.error("Deleting asset details failed with error: ", err);
...
exports.list = function(req, res) {
    ...
    DBModel.findAndCountAll(
        ...
        winston.error("Getting assets details list failed with error: ", err);
        res.jsonp(err);
    });
};
...
exports.dataByID = function(req, res, next, id) {
    ...
    DBModel.find({
        ...
        if (!result) {
            ...
            req.assetsDetails = result;
            next();
            return null;
        }
    }).catch(function(err) {
        winston.error("Getting assets details data failed with error: ", err);
        return next(err);
    });
};
